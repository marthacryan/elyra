...
import { RUNTIMES_NAMESPACE, KFP_SCHEMA } from '@elyra/pipeline-editor';
import {
  pipelineIcon,
  runtimesIcon,
  RequestErrors
...
const initPropertiesInfo = async (): Promise<any> => {
  return await PipelineService.getRuntimeImages().catch((error: any): any =>
...
const extension: JupyterFrontEndPlugin<void> = {
  ...
  activate: async (
    ...
    pipelineEditorFactory.widgetCreated.connect(
      async (sender: any, widget: any) => {
        void tracker.add(widget);

        const runtimeImages = await initPropertiesInfo();
        widget.initPropertiesInfo(runtimeImages);
